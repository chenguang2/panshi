...
| P1 两派收敛（1–2 周/节奏随迭代） | 双倍留白页面逐页清理；野值字号/间距替换为令牌；`--p-*` 冻结 | 每页前后截图对比入库 `docs/refactoring/`；jscpd 块数下降曲线 |
| P2 组件与效率（2–4 周） | 批量操作条、筛选器规范落地到 Top5 高频列表页；Dashboard 工作台化；全局搜索（菜单级） | 走查清单全绿 |
| P3 深色模式（单独立项） | 见第 7 节 | 双主题走查 |
