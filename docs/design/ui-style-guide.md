...
| P1 两派收敛 | ✅ 已完成：留白统一（19 页 288→264）、`--p-*` 别名清零（147 处迁回正源并删除别名块）、野值字号清零（15px 24 处→14px；9/10px 微字 61 处→11px，涉及徽章/分组签/侧栏标题，双视口截图核验无破版）。**剩余两项已降级为搭车治理**（2026-09-17 用户确认，不立专项）：① 视图内颜色字面量——日常改到哪页顺手收编那页，将来换品牌色/加第二主题时升级为必要；② jscpd css 重复块——需求/bug 恰好命中重复对时顺手合并该对。基线（2026-09-17 全仓）：css 147 块 / 2335 行（5.12%） | 前后截图对比 `/tmp/opencode/ui-effect/`（临时，不入库） |
| P2 组件与效率 | ✅ 已完成：全局搜索 Ctrl+K（`router/navMeta.ts` 索引 + `GlobalSearch.vue` 命令面板，7 项单测 + Playwright 链路实测；菜单命名与面包屑同源化）；**Dashboard 工作台化**（概览页重排：4 张运维统计卡 auto-fit 等宽 → 集群状态 + 节点在线卡（离线节点明细/全在线绿条）→ 最近路由 → 快捷入口/更多资源链接卡；数据全复用现有端点，零后端改动）。**筛选器盘点结论**（2026-09-17）：各列表页均为"变化即触发"式，符合 §2.3 规范，无需补查询/重置按钮；**批量操作**：已存在于 6 个有运维诉求的页面（节点任务/Ansible 清单/Edge 导入/ClusterNodes/DNS代理/四层代理），向配置列表页（路由/上游/SSL 等）扩展属产品决策，挂起待需求 | 走查清单全绿 |
