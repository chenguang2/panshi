...
| P2 组件与效率 | ✅ 已完成：全局搜索 Ctrl+K（`router/navMeta.ts` 索引 + `GlobalSearch.vue` 命令面板，7 项单测 + Playwright 链路实测；菜单命名与面包屑同源化）。⏳ 待续：批量操作条、筛选器规范推广到 Top5 高频列表页、Dashboard 工作台化 | 走查清单全绿 |
