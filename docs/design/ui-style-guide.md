...
| P1 两派收敛 | ✅ 已完成：留白统一（19 页 288→264）、`--p-*` 别名清零（147 处迁回正源并删除别名块）、野值字号清零（15px 24 处→14px；9/10px 微字 61 处→11px，涉及徽章/分组签/侧栏标题，双视口截图核验无破版）。⏳ 待续（按 #98 逐对治理）：视图内颜色字面量收敛；jscpd 基线（2026-09-17 全仓）：css 147 块 / 2335 行（5.12%） | 前后截图对比 `/tmp/opencode/ui-effect/`（临时，不入库） |
