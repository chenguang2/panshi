...
**边缘客户端的影响评估与结论**：实测 `scrollWidth == clientWidth`、无横向溢出，且该变化方向与 5/6 个 A 派页面一致（属一致性改善）。**用户已于 2026-09-16 确认保留 `nowrap`**，不回退——即"边缘客户端单元格单行显示"是**有意的既定行为**，后续会话勿以"零变更"为由把它改回 `normal`；若将来某列因超长内容需要折行，应针对该列单独处理（加 `ellipsis` 或按列覆写），而非全局回退。

**该页另有一处小差异（未改，待定）**：其局部规则的 `padding` 带 `!important`，因此 `th` 内边距是 **10/16**，而其余 6 个 A 派页面因 AntDV `middle` 覆盖实际渲染为 **12/8**（差 2px）。是否对齐待定。

**回归**：`vue-tsc -b` 通过；全量 **844/844 通过**；`npm run build` ✓。

### 6.2 A 派 7 处重复 CSS 收敛到全局一份（2026-09-16 完成）

**依据**：逐条抽比对后确认——A 派 7 个页面**各自复制了完全相同的 7 条规则**（外壳 / 表格透明 / 表头 / `th::before` / 行 / hover / 分页脚注），仅类名前缀（`.upstream-table`、`.audit-table`…）与个别 `!important` 不同。复制粘贴已造成三次实际漂移（审计页漏抄、边缘客户端内边距变体、2026-09-16 删块导致另一处静默失去间距）。

**改动（净减重复，非新增拷贝）**：删除 7 个页面共 **48 条规则 / 346 行**；全局 `style.css` 补 1 条**分页脚注**规则（原先 7 份拷贝里唯一未被全局覆盖的一条）。页面里 6 个 `-table` 类名保留为"按页覆写的锚点"（现无规则引用，属有意留白）。

**验证：计算样式 before/after 全量 diff**（7 页 × 外壳/表头/行/分页四区块，逐字段比对）

| 结果 | 内容 |
|---|---|
| **仅 5 处差异，且全部是预期的内边距统一** | 节点管理/节点任务/边缘客户端 `td` 左内边距 16→8px；边缘客户端 `th` 上内边距 10→12px、左 16→8px |
| **其余逐字段无变化** | 外壳背景/边框/圆角/溢出/阴影、表头背景/字号/大写/字距/颜色/换行、行字号/换行/边框、分页脚注全部四项、表格数量与外壳归属 |
