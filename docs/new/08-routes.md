...
> 本章场景：前面已经准备好了上游（第 7 章）、插件组（第 6 章）和插件元数据（第 5 章）。现在创建一条路由把它们串起来：请求 `/api/*` 且到达指定端口时，转发到"本地测试服务"，同时执行自定义预处理逻辑并按统一格式记录日志。
...
3. 填写基础配置：
   ...
   |---|---|---|
   | 名称 | `API路由` | 路由的唯一标识 |
   | URI | `/api/*` | 匹配路径，`*` 为通配符；必须以 `/` 开头 |
   | 所属集群 | 【演示集群】 | 创建后不可修改 |
   | 请求方法 | 全选 | 至少选择一种；点击方法标签进行勾选 |
   | 上游 | 【本地测试服务】 | 第 7 章创建的上游，必填 |
...
3. 点击卡片选中该插件。如需自定义逻辑，点击已选插件右侧编辑图标（✏️），在对应阶段填入 Lua 函数。本例在 `rewrite` 阶段添加一条示例：
...
![勾选日志插件组](images/08-07-route-plugingroup-selected.png)

# 创建

点击右下角【保存】按钮。创建完成后，列表中出现"API路由"，状态为"未发布"。

![创建成功](images/08-08-route-created.png)
...
![选择发布节点](images/08-09-route-publish-nodes.png)

3. 点击【确认发布】，等待发布结果。

![发布结果](images/08-10-route-publish-result.png)
...
```bash
curl -s http://localhost:12344/api/v1/routes?cluster_id=<集群ID> \
  -H "Authorization: Bearer <token>"
```

预期结果：

- 返回列表中包含名称为"API路由"的记录；
- 其 `uri` 为 `/api/*`，`upstream_id` 指向"本地测试服务"；
- 高级匹配变量包含 `server_port == 16610`；
- 路由插件包含 `pre_functions`；
- 已关联"日志插件组"；
- 列表行状态为"已发布"。

3. 端到端验证（可选）：向 Edge 节点的 **16610 端口**访问 `/api/*` 路径，请求应被转发到本地测试服务并返回其内容，同时在节点上生成按统一格式的日志；换其他端口访问则不命中本路由。
