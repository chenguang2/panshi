...
> 本章场景：平台的监控指标（Edge 节点采集的性能数据）存储在 ClickHouse 中。【ClickHouse 配置】管理指标查询使用的 ClickHouse 连接——支持多条命名连接，激活其中一条。此处切换仅影响监控/指标数据源，与平台数据库（第 21 章「数据库管理」）无关。
...
- 第 17 章「指标总览与指标查询」使用此处激活的连接执行查询
- 两者是**配置与消费**的关系：本页配连接，第 17 章用连接
