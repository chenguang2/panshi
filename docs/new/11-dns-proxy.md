...
> 本章场景：客户端需要通过域名访问网关服务。DNS 代理监听节点的 UDP 53 端口，把 `test.com` 解析到三个网关节点的地址上，并按一致性哈希返回。本章同时启用健康检查（HTTPS 探测），保证解析结果只包含存活节点。

# DNS 代理是什么

- **作用**：把指定域名解析为一组节点 IP 返回给客户端，实现"域名 → 网关集群"的流量引导。
...
![向导第一步](images/11-02-dns-wizard-step1.png)

2. 勾选【手动输入端口】，输入 `53`（DNS 标准端口，UDP）。
3. 点击【下一步】。
...
![第二步填写完成](images/11-03-dns-step2-filled.png)

> 💡 健康检查的 `type` 决定探测方式：`http` / `https` / `tcp` 等。本例节点 16610 是 HTTPS 端口，所以用 `https`；`active`（主动探测）与 `passive`（被动统计）参数保持默认即可。

7. 点击【创建】。列表出现"test.com 解析"卡片，状态为"未发布"。

![创建成功](images/11-04-dns-created.png)
...
![选择发布节点](images/11-05-dns-publish-nodes.png)

3. 点击【确认发布】，等待发布结果。

![发布结果](images/11-06-dns-publish-result.png)
...
2. 通过 API 确认 DNS 代理内容：

```bash
curl -s http://localhost:12344/api/v1/clusters/<集群ID>/dns-proxies \
  -H "Authorization: Bearer <token>"
```

预期结果：

- 返回列表中包含名称为"test.com 解析"的记录；
- 其监听端口为 `53`、协议为 `udp`；
- 域名映射含 `test.com`，负载均衡类型为 `chash`（一致性哈希），目标节点为三个 `16610`；
- 健康检查 JSON 的 `type` 为 `https`；
- 卡片状态为"已发布 v1"。

3. 端到端验证（可选）：在能连通节点的机器上执行：
