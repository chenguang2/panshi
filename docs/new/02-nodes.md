...
| 服务端口 * | 80 | OpenResty 对外提供 HTTP 服务的端口（默认 80），用于连通性探测和流量验证 |
| 管理端口 * | 16620 | Edge 管理 API 的端口（即 edge.env 中 `deploy.http.admin.listen`），**发布配置走这个端口**。表单默认 9180，请按节点实际部署修改 |
