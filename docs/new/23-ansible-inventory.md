...
点击左侧侧边栏：【工具箱】→【Ansible 主机清单】。

> 若菜单不可见：回[第 0 章 功能开关前置检查](00-login.md)确认 `ansible_inventory` 开关已启用。
...
若主机条目包含已知清单外的字段（非上述 11 个常用字段），行内显示「含自定义字段」标签——这些字段在保存时**全保真保留**，不会丢失。
...
若清单中存在尚未录入节点管理的 IP，页面顶部显示提示条：「以下 N 个 IP 存在于主机清单，但尚未录入节点管理」，附【全部导入】按钮一键录入。
...
**保存前自动备份**：每次保存前，系统自动备份当前文件（备份路径：`ansible/inventory/host.bak`）。

# 与其他页面的关系

| 需求 | 推荐入口 |
|---|---|
| 管理单个集群的节点 | 【节点管理】（第 2 章） |
| 管理 Edge 节点的 SSH 连接配置 | 本页 |
| 查看节点执行任务的日志 | 【节点管理】→ 任务日志 |
| 管理平台级 SSH 配置 | 【Edge 环境配置】（第 3 章） |
