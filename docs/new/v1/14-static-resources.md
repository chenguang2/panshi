...
创建前需要一条**可用的路由**（第 8 章已学过创建方法），且同时满足三个校验项：
...
1. 在列表中找到刚创建的资源行，点击【上传 ZIP】，选择本地 ZIP 文件。
2. 提示「上传成功」后，该行显示文件大小。

📷 截图待补充：（上传成功提示，资源行显示文件大小）

3. 点击【查看】可浏览 ZIP 包内的文件清单，确认内容正确。
...
1. 点击该行的【发布】按钮（未上传 ZIP 时置灰）。
2. 弹出【发布静态资源】确认框，选择节点后确认。
3. 发布进度实时显示每台节点的执行日志：
   - 全部成功 → 状态徽章变为 **已发布**；
   - 部分失败 → 显示 ⚠️ 提示，修复对应节点后重新发布即可（排查思路见[第 3 章 3.4](03-edge-env.md)）。
...
1. 列表中该资源状态徽章为 **已发布**；
2. 通过关联的路由路径访问 ZIP 内文件（把 `test.com:5000` 换成你的实际域名/端口与路由 URI）：
...
下一步：[第 15 章 DNS代理[HTTP]](15-dns-http.md)
