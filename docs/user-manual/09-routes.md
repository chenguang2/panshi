...
> 本章场景：前面已经准备好了上游（第 8 章）、插件组（第 7 章）和插件元数据（第 6 章）。现在创建一条路由把它们串起来：请求 `/api/*` 且到达指定端口时，转发到"测试服务"，同时执行自定义预处理逻辑并按统一格式记录日志。
...
- **转发目标**：引用一个上游（第 8 章），由上游决定后端节点与负载均衡。
- **附加处理**：路由级插件 + 关联的插件组（第 7 章），插件元数据（第 6 章）提供全局属性。
...
1. 切换到【插件组】标签页。
2. 勾选【日志插件组】（第 7 章创建）。插件组的配置会合并到路由中——本路由的请求将按第 6 章元数据定义的统一格式记录日志。
...
下一步：[9. 根证书与证书签发](10-certificates.md)
