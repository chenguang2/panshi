...
- **左侧侧边栏**：功能入口，按分组排列
  - 【核心功能】：集群管理、节点管理、上游管理、路由管理、插件组、插件元数据、全局规则等（第 2~10 章）
  - 【边缘网络】：edge.env 配置、四层代理、DNS代理[UDP]（第 4、11、12 章）
  - 【综合】：统一管理、指标查询、指标总览（第 17~18 章）
  - 【系统管理】：插件开关、数据库管理、用户管理（第 19~21 章）
  - 【运维管理】：Edge直连、数据导入、工具箱、自启动管理、节点任务、Ansible 主机清单（第 22~24 章）
...
| Ansible 主机清单 | 运维管理 | `ansible_inventory` |
