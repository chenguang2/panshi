...
- 「开始迁移」按钮禁用，防止重复发起迁移
- 超时时间下拉框禁用
- 页面右上角【刷新】按钮可手动刷新迁移状态

> 💡 迁移进度在页面刷新后不会丢失——后端会持久化进度信息，重新进入页面时自动恢复显示。5 秒自动轮询确保进度实时更新。
...
数据迁移卡片底部展示**历史迁移记录**表格，自动加载并支持分页浏览：
...
| 刷新页面后迁移进度消失 | 现已修复——后端持久化迁移进度，刷新页面后自动恢复显示 |
| 迁移完成后历史记录为空 | 迁移完成后会自动刷新历史记录；若仍未显示，点击【刷新】按钮 |
| 想同时发起多个迁移 | 不支持——有迁移进行中时，「开始迁移」按钮自动禁用 |
