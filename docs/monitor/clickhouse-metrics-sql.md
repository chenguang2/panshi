...
> ⚠️ 仅当该指标**只有一条序列**时结果正确。序列 = 指标名 + 唯一标签组合
> （`Attributes`），代表一个独立上报源；与图表上"平均值/最大值"两条折线**无关**
> （那是同一条序列的两种统计量）。当前 `edge_http_requests_total` 只有 1 条序列：
> `{'ip': '192.168.100.42'}`；再加一台 Edge 节点即变 2 条，`lagInFrame` 会跨
> 序列取上一行，差值失去意义。判断方法：对该指标 `GROUP BY Attributes` 数组合数
> （`edge_http_status` 就是"近 10 分钟 1 条、24 小时很多条"的动态例子）。
> 更隐蔽的边界：若多个节点上报的标签完全相同，两个版本都无法区分，需先在上报侧补区分标签。
