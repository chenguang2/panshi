...
为四层代理卡片列表页（普通 TCP/UDP/TLS 与 DNS 代理）提供批量删除能力：批量管理模式、勾选多卡片、全选联动、聚合确认与逐条进度反馈，以及后端跨集群批量删除端点。
...
- **THEN** 「全选当前分组」SHALL NOT 显示

#### Scenario: 全选当前筛选结果
- **WHEN** 用户点击「全选当前筛选结果」且当前筛选/搜索/分组条件下存在未选中的卡片
- **THEN** 当前全部**已加载**卡片 SHALL 被选中（范围 = 当前页已加载数据）
- **WHEN** 当前全部已加载卡片已选中时再次点击
- **THEN** 当前全部已加载卡片 SHALL 被取消选中（toggle）

#### Scenario: 筛选变化保留选择
- **WHEN** 用户已勾选部分卡片后更改搜索词/集群/分组筛选
- **THEN** 已选中的卡片 SHALL 按 id 保留，计数与确认弹窗 SHALL 基于原始代理列表（proxies 快照）解析，不受当前筛选视图影响
...
- **THEN** 确认弹窗 SHALL 列出全部 N 个将删除的代理（名称 + 协议/端口摘要，基于原始列表解析）
- **THEN** 弹窗 SHALL 提供「同时从数据库删除」「同时删除 Edge 节点」选项与不可恢复警告
- **AND** SHALL NOT 要求输入名称确认（与集群删除不同）
- **AND** 勾选「删除 Edge 节点」时 SHALL NOT 提供逐节点选择——删除各集群**全部在线节点**上的配置，弹窗文案 SHALL 明示
- **WHEN** 用户确认
...
- **THEN** 返回 `{ message, results: [{proxy_id, name, status, results}] }`（`name` 为代理名称，对齐前端 nameField）
- **THEN** 单条失败 SHALL 以 `status: "failed"` + `message` 标记于 results 条目，不抛 HTTPException 中断整体
- **WHEN** 批量删除勾选「删除 Edge 节点」且未指定 `node_ids`
- **THEN** 系统 SHALL 删除各代理所属集群的**全部在线节点**上的配置
- **WHEN** `proxy_ids` 为空
...
- **THEN** 系统 SHALL 对两类代理均按相同批量流程删除（不按 proxy_type 过滤，按 id 精确删除）
