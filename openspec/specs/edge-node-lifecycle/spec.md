...
The system SHALL support reloading the process configuration via `POST /clusters/{cluster_id}/nodes/{node_id}/reload`（原 `/restart` 已改名）. Tag: `nginx_cmd_run`, extravar: `nginx_cmd: nginx_reload`.

前端 SHALL 在节点操作栏中显示"⟳ reload"按钮，替代原有"ⓘ 详情"按钮的位置。详情按钮移入更多菜单，放在编辑上方。

#### Scenario: Successful reload
- **WHEN** user sends `POST /clusters/1/nodes/5/reload`
