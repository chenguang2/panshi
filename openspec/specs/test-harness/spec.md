...
Provides isolated test fixtures for the backend pytest suite, ensuring all tests run against per-test in-memory SQLite databases with zero dependency on the real active database. This eliminates environment drift, SQLite WAL lock contention with the live backend, and unsafe writes to production data.
