...
Edge 节点已有自定义 Lua 插件框架（data_center、pre_functions、proxy_rewrite 等），可以通过新增 Admin API handler 和 PANSHI 插件来扩展能力。
...
3. **PANSHI 插件方式（非 Nginx 注入）** — 不需要 reload，完全在 PANSHI 请求生命周期内处理。
