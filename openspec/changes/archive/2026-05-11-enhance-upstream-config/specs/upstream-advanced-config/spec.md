...
- **AND** "高级配置" Tab 不可点击或显示提示

#### Scenario: 启用高级配置
- **WHEN** 用户开启"高级配置"开关
- **THEN** "高级配置" Tab SHALL 变为可点击
- **AND** 高级配置中的健康检查配置 SHALL 默认展开并使用预设默认值

### Requirement: 高级配置包含健康检查
高级配置 Tab SHALL 包含可编辑的健康检查（checks）配置。默认沿用现有硬编码的 checks JSON 作为初始值。

#### Scenario: 查看健康检查默认值
- **WHEN** 用户进入高级配置 Tab
- **THEN** 健康检查区域 SHALL 显示完整的 checks JSON
- **AND** 默认值 SHALL 包含 passive（type: http）和 active（type: http）配置
