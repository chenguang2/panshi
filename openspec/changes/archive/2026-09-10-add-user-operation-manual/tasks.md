...
- [x] 1.1 健康检查前后端（12344/12345）
- [x] 1.2 数据库管理中新增/修改 SQLite 连接指向 `data/manual-demo.db` 并设为当前，重启后端
- [x] 1.3 验证空库状态（仅 admin 账号、无集群）

## 2. 截图基建

- [x] 2.1 建立 Playwright 截图脚本（登录态复用、统一视口、输出 docs/user-manual/images/）
- [x] 2.2 登录页与主界面导览截图（00-*）
...
- [x] 4.2 全部截图占位清单核对（images/ 引用有效性）
- [ ] 4.3 恢复原数据库配置并重启，确认开发数据完好
- [x] 4.4 openspec 工件勾选完成、变更可归档
