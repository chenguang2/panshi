...
Edge 数据导入路由时，`convert_route()` 方法仅提取基础字段（uri、methods、hosts、priority、upstream_id、plugins），未处理 PANSHI route 中的高级匹配字段 `vars`、`remote_addrs`。Route ORM 模型和前端表单已支持这些字段，但导入管道未覆盖。
...
| `remote_addrs` 格式 | PANSHI 返回 list → join 为逗号分隔字符串 | 与 `hosts`、`methods` 处理方式一致，Route 模型 `remote_addrs` 为 `String(500)` |

## Risks / Trade-offs

- [兼容性] 已有导入记录不包含这些字段，属正常行为（导入时 missing 字段默认为 NULL/0）
- [PANSHI 差异] 如果 Edge 返回的 `vars` 格式与预期不符（如非标准三元组），`json.dumps` 仍能存入，前端展示时不会崩溃（由 `convert_vars` validator 容错）
