...
- [ ] 1.4 编写单元测试：验证 SQLite boolean 0/1 → PostgreSQL True/False 转换
- [ ] 1.5 编写单元测试：验证 SQLite datetime string → PostgreSQL datetime object 转换
...
- [ ] 2.4 编写单元测试：验证大表（1000+行）分批读取不 OOM
- [ ] 2.5 编写单元测试：验证空表迁移不报错
...
- [ ] 4.5 编写单元测试：验证返回结构包含每张表的详细信息

## 5. 集成测试和清理

- [ ] 5.1 更新 `test_sqlite_to_pg_migration.py`，验证类型转换在实际迁移中生效
- [ ] 5.2 更新 `test_database_api.py`，验证新的 API 返回结构
- [ ] 5.3 端到端测试：SQLite → PostgreSQL 完整迁移验证
- [ ] 5.4 端到端测试：PostgreSQL → SQLite 完整迁移验证
- [ ] 5.5 更新前端 `database.ts` 类型定义，适配新返回结构
