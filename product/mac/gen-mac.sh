...
# ======================================================
# 部署准备脚本 (macOS)
# 功能：在开发机上准备好所有依赖，生成离线部署包到 product/mac/panshi/
# 要求：开发机需安装 uv 和 npm（有公网访问）
# 用法：bash product/mac/gen-mac.sh
# ======================================================
set -euo pipefail
...
echo ""
echo "[3/5] 创建虚拟环境..."
# macOS 上的 cp -rL 拷贝的 Python，venv --copies 创建后 .venv/bin/python3 无法找到
# libpython3.11.dylib（@rpath 解析为 .venv/lib/），需要手动拷贝该 dylib
"$TARGET_PYTHON_DIR/bin/python3" -m venv --copies --without-pip "$TARGET_DIR/backend/.venv"
mkdir -p "$TARGET_DIR/backend/.venv/lib"
cp "$TARGET_PYTHON_DIR/lib/libpython3.11.dylib" "$TARGET_DIR/backend/.venv/lib/"
# 通过 .venv 内 python 安装 pip
"$TARGET_DIR/backend/.venv/bin/python3" -m ensurepip --upgrade --default-pip
echo "  .venv 已创建"
...
cp "$PROJECT_ROOT/backend/ansible/collections/requirements.yml" "$TARGET_DIR/backend/ansible/collections/requirements.yml" 2>/dev/null || true
...
echo ""
echo "部署步骤："
echo "  1. 将 product/mac/panshi/ 目录拷贝到目标 macOS 机器"
echo "     scp -r product/mac/panshi user@target:~"
