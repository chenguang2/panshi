...
30. **测试套件治理基线** — pytest 已配 `--timeout=90`（pyproject）；`tests/api_helpers.py` 的 `isolated_app_lifespan()` 用于隔离 lifespan 真实库/后台服务交互；`_no_edge_http` 模式（autouse mock EdgeClient 网络）杜绝测试真实网络 IO。45 个文件仍绑定真实库（迁移立项 `openspec/changes/test-suite-db-isolation`），全量审计见 `docs/refactoring/test-suite-consolidation-2026-09-12.md`。禁新增：`assert callable/hasattr` 型零价值用例、跨文件复制同行为用例（单字段变体一律 parametrize）。**⚠️ 2026-09-17 起：活动库切 PostgreSQL 后禁止跑全量 `uv run pytest`** —— `isolated_app_lifespan` 只隔离 lifespan，不重定向引擎，测试会直连真实 PG，asyncpg 不容忍跨事件循环/并发复用连接（实测全量 102 failed、71 次登录即 `InterfaceError: another operation is in progress`），且存在向真实库写入的风险（本次取证：失败集中在 setup 阶段，未造成污染）。单文件/定向跑正常。test-suite-db-isolation 完成前，后端回归用「定向文件 + live 冒烟」；该立项从技术债升级为安全隐患，优先级应提级。
