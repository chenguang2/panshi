...
- **数据库管理** — 支持 SQLite / PostgreSQL 双后端：连接测试、设为当前（重启生效）、双向快照迁移、归档导出/导入
...
支持 SQLite 与 PostgreSQL 双后端。开发模式默认使用本地 SQLite，无需额外安装数据库；生产环境可切换至 PostgreSQL。

### 配置方式

1. 登录后进入 **系统管理 → 数据库管理**
2. 点击「添加连接」，填写 PostgreSQL 主机 / 端口 / 数据库名 / 用户名密码（或添加其他 SQLite 文件路径）
3. 行内「测试」验证连通性（3 秒超时）
4. 「设为当前」并按提示重启后端服务——切换前旧配置自动备份为 `db_config.json.bak`，若新库启动失败会自动回滚

激活连接配置存储于 `backend/data/db_config.json`（权限 600，密码 Fernet 加密）。切换数据库后 JWT 会话保持有效，无需重新登录；目标库为空时仅保留 admin 账号。

### 数据迁移

在数据库管理页「数据迁移」卡片执行 **源 → 目标** 单向快照迁移：

- **替换模式**：清空目标库后全量写入（非空库需勾选确认，自动先导出归档备份）
- **合并模式**：不清空目标库，将源数据并入
- 可选是否包含日志类数据（审计日志、导入日志等）
- 迁移期间全局写锁定（写请求返回 503），完成或失败后自动解除
- 支持归档导出（zip：meta + schema + JSONL 数据）与归档导入还原

> 静态资源文件存储于服务器磁盘，迁移 / 切换数据库不影响已上传的静态资源。
