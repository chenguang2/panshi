...
2. **修正标题层级**：
   - 保留唯一 H1（文档标题）
   - 章节降为 H2，子节降为 H3
3. **分配标题编号**（编号放 md，已确认的设计决策）：
   - H1 = 章节号：`# 0. 登录与初始准备`
   - H2 = 章节.节：`## 0.1 登录`
   - H3 = 章节.节.子节：`### 0.4.1 新增 SQLite 连接`
   - 编号格式与模板 Word 自动编号一致（1、1.1、1.1.1），但由 md 手动维护
4. **修正其他问题**：裸 URL、表格分隔行、列表空行、行尾空格等
5. **验证**：
...
| 标题无编号 | 编号未维护 | 按 `章节.节.子节` 分配（如 `0.1`、`0.4.1`） |
| 裸 URL 报错 | 未用尖括号 | `<http://...>` |
| 表格报 MD060 | 分隔行无空格 | `| --- | --- |` |
| 引用块内列表报 MD032 | 列表前无空行 | `> 文字` + `>` + `> - 列表项` |
| 引用块连续行转 Word 合并成一行 | 连续 `>` 行是同一段落，pandoc 把软换行渲染成空格 | 给非列表项的连续引用行加两个尾随空格（硬换行），如 `> 文字  ` |
