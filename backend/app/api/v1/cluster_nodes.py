...
# Registry mapping node_id -> SSH subprocess for install-openresty (used by cancel-install)
...
async def _install_openresty_stream(
    ...
    try:
        ...
        try:
            ...
            while True:
                # Explicitly check if client disconnected (more reliable than GeneratorExit)
...
@router.post("/{cluster_id}/nodes/{node_id}/cancel-install")
async def cancel_install(
    ...
    # Step 3: Pkill if still alive
    if any("install-edge.sh" in l for l in ps_lines):
        rc, pkill_stdout, pkill_stderr = await _ssh_run(node.ip, "pkill -f install-edge.sh")
        steps.append({
