...
async def publish_global_rule(cluster_id: int, rule_id: int, req: Optional[PublishRequest] = None, db: AsyncSession = Depends(get_db)):
    ...
    return await edge_sync.publish_resource(
        db, cluster_id=cluster_id, resource=rule, resource_type="global_rule",
        config_data=config_data, edge_data=edge_data,
        publish_fn=lambda client: client.create_global_rule(rule.edge_uuid, edge_data),
        display_name="全局规则",
        log_path=f"/edge/admin/global_rules/{rule.edge_uuid}",
        log_resource_id=rule_id, log_resource_name=rule.name,
...
async def get_global_rule_history(cluster_id: int, rule_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return await edge_sync.list_config_versions(
        db, resource_type="global_rule", resource_id=rule_id,
        current_version=rule.current_version if rule else None)
...
async def delete_global_rule_history(cluster_id: int, rule_id: int, history_id: int, db: AsyncSession = Depends(get_db)):
    await edge_sync.delete_config_version(db, resource_type="global_rule", resource_id=rule_id, history_id=history_id)
