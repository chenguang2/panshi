...
class TestMetricsAPI:
    ...
    def test_get_metric_names(self, mock_query, isolated_app):
    ...
    @patch("app.api.v1.metrics.query_metric_names")
    def test_get_metric_names_empty(self, mock_query, isolated_app):
    ...
    def test_get_metric_time_series(self, mock_query, isolated_app):
        mock_query.return_value = [
            {"metric_name": "cpu", "timestamp": 1000, "avg": 50.0, "max": 80.0, "min": 30.0, "sample_count": 6},
        ]
        resp = isolated_app.get("/api/v1/metrics/cpu")
        assert resp.status_code == 200
        body = resp.json()
        assert body["data"][0]["avg"] == 50.0

    @patch("app.api.v1.metrics.query_time_series")
    def test_get_metric_time_series_with_params(self, mock_query, isolated_app):
        mock_query.return_value = []
        resp = isolated_app.get("/api/v1/metrics/cpu?since=6h&interval=15m&label=state:active")
        assert resp.status_code == 200
        mock_query.assert_called_once_with(metric_name="cpu", since="6h", interval="15m", label="state:active")

    @patch("app.api.v1.metrics.query_time_series")
    def test_get_metric_time_series_empty(self, mock_query, isolated_app):
        mock_query.return_value = []
        resp = isolated_app.get("/api/v1/metrics/cpu")
        assert resp.status_code == 200
        assert resp.json() == {"data": []}
    ...
    def test_get_summary(self, mock_query, mock_states, isolated_app):
        mock_query.return_value = {"cpu": 85.0, "mem": 1024.0}
        mock_states.return_value = {"active": 5.0, "waiting": 3.0}
        resp = isolated_app.get("/api/v1/metrics/summary")
        assert resp.status_code == 200
        assert resp.json() == {
    ...
    @patch("app.api.v1.metrics.query_connection_states")
    @patch("app.api.v1.metrics.query_summary")
    def test_get_summary_empty(self, mock_query, mock_states, isolated_app):
        mock_query.return_value = {}
...
class TestMetricsAPINonBlocking:
    ...
    @pytest.mark.asyncio
    async def test_slow_clickhouse_does_not_block_other_requests(self, async_authed_client):
        import asyncio
        import threading
        import time
        from unittest.mock import patch
