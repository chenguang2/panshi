...
async def _logs(session_factory):
    """直接查询审计日志（使用 async_isolated_client 同库的 session factory）。"""
    async with session_factory() as s:
        return (await s.execute(select(AuditLog).order_by(AuditLog.id))).scalars().all()


@pytest.mark.asyncio
async def test_route_create_delete_enrich_detail(async_authed_client, isolated_session):
    ...
    r2 = await c.request("DELETE", f"/api/v1/clusters/1/routes/{route_id}",
                         json={"delete_db": True, "delete_edge": False})
    assert r2.status_code in (200, 204)

    logs = await _logs(isolated_session)
...
@pytest.mark.asyncio
async def test_route_update_detail_has_before_after(async_authed_client, isolated_session):
    c = async_authed_client
    # 直接播种路由
    ...
    logs = [x for x in await _logs(isolated_session) if x.action == "route_update"]
    assert len(logs) == 1
...
@pytest.mark.asyncio
async def test_batch_delete_single_row_with_id_list(async_authed_client, isolated_session):
    c = async_authed_client
    # 直接播种 3 条路由
    async with isolated_session() as s:
    ...
    r = await c.request("DELETE", "/api/v1/clusters/1/routes",
                        json={"route_ids": [21, 22, 23], "delete_db": True, "delete_edge": False})
    assert r.status_code in (200, 204), r.text

    logs = [x for x in await _logs(isolated_session) if x.action == "route_batch_delete"]
...
@pytest.mark.asyncio
async def test_legacy_explicit_audit_dedupes_with_skeleton(async_authed_client, isolated_session):
    """clusters.py 创建集群仍走旧式显式 log_audit —— 与骨架去重后仅 1 条。"""
    c = async_authed_client
    r = await c.post("/api/v1/clusters", json={"name": "new-cluster"})
    assert r.status_code in (200, 201), r.text

    logs = [x for x in await _logs(isolated_session) if x.resource == "cluster"]
...
@pytest.mark.asyncio
async def test_upstream_crud_enrich_detail(async_authed_client, isolated_session):
...
@pytest.mark.asyncio
async def test_ssl_delete_enrich_detail(async_authed_client, isolated_session):
    """SSL 证书删除审计详情带证书名。"""
    async with isolated_session() as s:
