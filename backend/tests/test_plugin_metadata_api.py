...
class TestPluginMetadataListAPI:

    async def test_list_all_returns_data(self, async_authed_client):
        response = await async_authed_client.get("/api/v1/plugin_metadata")
    ...
    async def test_list_plugin_metadata_group_filter(self, async_authed_client):
        response = await async_authed_client.get(
            "/api/v1/plugin_metadata",
