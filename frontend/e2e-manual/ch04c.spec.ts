...
test('04 global rule full flow', async ({ page }) => {
  ...
  await overlay.getByPlaceholder('请输入名称').fill('全局链路追踪')
  ...
  // 切到插件配置 tab
  await overlay.locator('.modal-header ~ *, .modal-body *').filter({ hasText: '插件配置' }).first().click().catch(() => {})
  await page.getByText('插件配置', { exact: true }).click()
