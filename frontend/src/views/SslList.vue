    ...
    <div v-else class="ssl-grid">
      <div v-for="cert in displayedCerts" :key="cert.id" class="ssl-card" :style="getCardBorderStyle(cert.cluster_group_name)">
        ...
        <div class="ssl-card-body">
          ...
          <div class="ssl-card-row"><label>类型</label><span>{{ cert.cert_type }}<span v-if="cert.is_ca" class="badge badge-secondary" style="margin-left:6px;font-size:10px;">CA 根证书</span><span v-if="cert.is_ca && cert.algorithm === 'sm2'" class="badge algo-sm" style="margin-left:4px;font-size:10px;">SM2</span><span v-if="cert.is_ca && cert.algorithm === 'rsa'" class="badge algo-international" style="margin-left:4px;font-size:10px;">RSA 2048</span><span v-if="cert.is_ca && cert.algorithm === 'ecc'" class="badge algo-international" style="margin-left:4px;font-size:10px;">ECC P-256</span><span v-else-if="cert.algorithm === 'sm2' && cert.sign_cert" class="badge algo-sm" style="margin-left:6px;font-size:10px;">🇨🇳 国密 SM2 双证书</span><span v-else-if="cert.algorithm === 'sm2'" class="badge algo-sm" style="margin-left:6px;font-size:10px;">🇨🇳 国密 SM2 单证书</span><span v-else-if="cert.algorithm === 'rsa'" class="badge algo-international" style="margin-left:6px;font-size:10px;">🌐 国际 RSA 2048</span><span v-else-if="cert.algorithm === 'ecc'" class="badge algo-international" style="margin-left:6px;font-size:10px;">🌐 国际 ECC P-256</span><span v-if="cert.create_method === 'local_generate'" class="badge badge-secondary" style="margin-left:4px;font-size:10px;">本地生成</span></span></div>
