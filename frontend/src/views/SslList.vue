  ...
  <div class="ssl-page">
    <PageHeader title="SSL 证书" description="管理 Edge 网关的 SSL 证书">
      <template #actions>
        ...
        <button v-if="activeTab === 'ca'" class="btn btn-primary" style="margin-left: 8px" @click="openCaCreateDialog">
          + 创建 CA 根证书
        </button>
      </template>
    </PageHeader>
    ...
    <div v-else class="ssl-grid">
      <div
        v-for="cert in displayedCerts"
        :key="cert.id"
        class="ssl-card"
        :style="getCardBorderStyle(cert.cluster_group_name)"
      >
        <div class="ssl-card-topbar" :style="getGroupColorStyle(cert.cluster_group_name)">
          ...
          <span
            v-if="cert.is_ca"
            class="algo-badge algo-sm"
            style="
              background: oklch(55% 0.18 160/18%);
              color: oklch(40% 0.2 160);
              border-color: oklch(55% 0.18 160/30%);
            "
            >CA 根证书</span
          >
        ...
        <div class="ssl-card-header">
          ...
          <div class="ssl-card-meta">
            ...
            <span v-else-if="cert.current_version" class="badge badge-success"
            ...
            <div class="ssl-version-text">
              <template v-if="cert.current_version"
                ><PublishStatusTag :version="cert.current_version" :published-at="cert.published_at"
              /></template>
            </div>
          </div>
        </div>
        <div class="ssl-card-body">
          <div class="ssl-card-row">
            <label>SNI</label
            ><span class="ssl-sni-tags">
              <template v-for="s in splitSniString(cert.sni)" :key="s">
                <span class="sni-tag" :class="{ 'sni-tag-locked': isReservedSni(s) }"
                  >{{ s }}<span v-if="isReservedSni(s)" class="sni-tag-reserved">系统保留</span></span
                >
              </template>
        ...
        <div class="ssl-card-actions">
          ...
          <button v-if="!cert.is_ca" class="btn btn-ghost btn-sm ssl-action-btn" @click="openEditDrawer(cert)">
            编辑
          </button>
          <button class="btn btn-ghost btn-sm ssl-action-btn" style="color: var(--danger)" @click="deleteCert(cert)">
            删除
          </button>
          <span style="flex: 1"></span>
          <button
            v-if="!cert.is_ca && cert.cert_type !== 'client'"
            class="btn btn-secondary btn-sm"
            @click="publishCert(cert)"
          >
            发布
          </button>
          <button v-if="!cert.is_ca" class="btn btn-secondary btn-sm" @click="openVersionManagement(cert)">
            版本管理
          </button>
        </div>
      </div>
    ...
    <SslGenerateDialog
      ...
      @open-ca-create="openCaCreateDialog"
    />

    <CaCreateDialog
      :visible="caCreateVisible"
      :clusters="clusters"
      @close="caCreateVisible = false"
      @success="onCaCreateSuccess"
    />
...
async function loadCerts() {
  ...
  try {
    ...
    const resolved = items.map((cert: any) => ({
      ...cert,
      cluster_name:
        clusterMap.get(cert.cluster_id)?.display_name ||
        clusterMap.get(cert.cluster_id)?.name ||
        String(cert.cluster_id),
      cluster_group_name: clusterMap.get(cert.cluster_id)?.group_name || '',
    }))
    certs.value =
      activeTab.value === 'ca' ? resolved.filter((c: any) => c.is_ca) : resolved.filter((c: any) => !c.is_ca)
    totalCount.value = certs.value.length
  } catch {
    certs.value = []
    totalCount.value = 0
    message.error('加载 SSL 证书失败')
  } finally {
    loading.value = false
  }
}
...
function openEditDrawer(cert: any) {
...
function openCaCreateDialog() {
  caCreateVisible.value = true
}
function openDownloadDialog(cert: any) {
  downloadCertData.value = cert
...
function openVersionManagement(cert: any) {
  vmId.value = cert.id
  vmClusterId.value = cert.cluster_id
  vmName.value = cert.name
