  ...
  <div class="pml-page">
    ...
    <div v-else class="pml-grid">
      <div
        ...
        :key="item.id"
        class="pml-card"
        :style="getCardBorderStyle(item.cluster_group_name)"
      >
          ...
          <div class="pml-card-meta">
            ...
            <div class="pml-version-text">
              <template v-if="item.current_version"
                ><PublishStatusTag :version="item.current_version" :published-at="item.updated_at"
              /></template>
            </div>
          </div>
        </div>
        <div class="pml-card-actions">
          ...
          <button class="btn btn-ghost btn-sm pml-action-btn" style="color: var(--danger)" @click="deleteItem(item)">
            删除
          </button>
    ...
    <a-drawer
      ...
      :title="`查看插件元数据 - ${viewingItem?.plugin_name}`"
      width="560"
      ...
      <div v-if="viewingItem">
        <a-descriptions :column="1" bordered size="small">
          ...
          <a-descriptions-item label="状态">
            <span v-if="viewingItem.current_version" class="badge badge-success"
              ><span class="status-dot online"></span>已发布</span
            >
        ...
        <pre class="config-preview">{{
          viewingItem.config_data ? JSON.stringify(viewingItem.config_data, null, 2) : '{}'
        }}</pre>
      </div>
    </a-drawer>
