...
export function deleteStreamProxy(
  clusterId: number,
  proxyId: number,
  data: { delete_db: boolean; delete_edge?: boolean; node_ids?: number[] },
) {
...
export function detectPorts(clusterId: number, nodeId: number, excludeProxyId?: number, excludePort?: number) {
  return api.post<{ ports: PortItem[] }>(`/clusters/${clusterId}/stream-proxies/detect-ports`, {
    node_id: nodeId,
    exclude_proxy_id: excludeProxyId,
