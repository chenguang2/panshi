  ...
  <div class="modal-overlay" :style="{ display: visible ? 'flex' : 'none' }">
    <div class="modal modal-wide" style="max-width: 680px">
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select
              ...
              class="form-input"
              :class="{ 'has-error': errors.cluster_id }"
              :disabled="generating"
        ...
        <div class="form-row">
          <div class="form-group" style="flex: 1">
            ...
            <a-select
              v-model:value="form.ca_cert_id"
              style="width: 100%"
              :disabled="generating || caCertsLoading"
              placeholder="请选择 CA 根证书"
            >
              <a-select-option v-for="ca in filteredCaCerts" :key="ca.id" :value="ca.id"
                >{{ ca.name }} ({{ ca.algorithm || 'sm2' }})</a-select-option
              >
            </a-select>
            <div v-if="filteredCaCerts.length === 0 && caCerts.length > 0" class="form-hint">
              该集群没有 {{ algorithmLabel }} 类型的 CA，<a
                style="cursor: pointer; text-decoration: underline"
                @click="$emit('openCaCreate')"
                >创建 {{ algorithmLabel }} CA</a
              >
            </div>
              ...
              <div v-show="mtlsExpanded" class="collapse-body">
                <div class="form-group">
                  ...
                  <textarea
                    ...
                    class="form-input"
                    rows="6"
                    placeholder="留空则自动使用当前 CA 的证书（勾选同时生成客户端证书时）&#10;-----BEGIN CERTIFICATE-----&#10;...&#10;-----END CERTIFICATE-----"
                  ></textarea>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input
              ...
              type="text"
              class="form-input"
              :class="{ 'has-error': errors.common_name }"
        ...
        <div class="form-group">
          ...
          <div class="sni-tag-input" :class="{ 'has-error': errors.dns_sans }" @click="dnsInputRef?.focus()">
            ...
            <input
              ...
              :disabled="generating"
              @keydown.enter.prevent="addDnsTag"
              @keydown="addDnsTagOnComma"
            />
          </div>
        ...
        <div class="form-group">
          ...
          <div class="sni-tag-input" :class="{ 'has-error': errors.ip_sans }" @click="ipInputRef?.focus()">
            ...
            <input
              ...
              type="text"
              class="sni-input-inline"
              placeholder="输入 IP 后按 Enter"
              :disabled="generating"
              @keydown.enter.prevent="addIpTag"
              @keydown="addIpTagOnComma"
            />
          </div>
        ...
        <div v-if="commandLog.length > 0" class="log-section">
          ...
          <div
            v-for="(entry, i) in commandLog"
            :key="i"
            class="log-entry"
            :class="{ 'log-error': entry.exit_code !== 0 }"
          >
            ...
            <div class="log-body">
              ...
              <div v-if="expandedLogs[i]" class="log-detail">
                ...
                <div v-if="entry.stderr" class="log-stderr">
                  错误输出:
        ...
        <div
          ...
          style="margin-top: 12px; padding: 12px; background: var(--bg); border-radius: var(--radius)"
        >
          <div style="font-size: 13px; font-weight: 600; margin-bottom: 8px">生成成功</div>
          <div style="font-size: 12px">
            服务端证书：<strong>{{ resultData.server?.name }}</strong>
          </div>
          <div v-if="resultData.client" style="font-size: 12px; margin-top: 4px">
            ...
            <button
              ...
              @click="downloadClientBundle(resultData.client)"
            >
              下载客户端证书包
            </button>
          </div>
      ...
      <div class="modal-footer">
        <button class="btn btn-ghost" @click="handleClose" :disabled="generating">
          {{ commandLog.length > 0 ? '关闭' : '取消' }}
        </button>
        <button
...
async function handleGenerate() {
  ...
  try {
    const result = await generateSslCertificate(Number(form.cluster_id), {
      ...
      skip_mtls_uri_regex:
        mtlsEnabled.value && form.algorithm === 'sm2' && mtlsSkipTags.value.length > 0
          ? JSON.stringify(mtlsSkipTags.value)
          : undefined,
    })
...
function removeIpTag(i: number) {
  ipTags.value.splice(i, 1)
}
...
function removeMtlsSkipTag(i: number) {
