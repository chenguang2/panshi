<template>
  <a-modal v-model:open="modalOpen" :title="editingConfig ? '编辑插件组' : '添加插件组'" width="800px" :confirm-loading="submitting" @ok="handleSubmit">
      ...
      <a-tab-pane key="basic" tab="基础配置">
        <a-form ref="formRef" :model="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
          <a-form-item label="名称" name="name" :rules="[{ required: true, message: '请输入插件组名称' }]">
            <a-input v-model:value="form.name" placeholder="请输入插件组名称" />
          </a-form-item>
