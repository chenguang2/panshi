    ...
    <div class="modal">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input
            type="text"
            class="form-input"
            :class="{ 'has-error': formErrors.display_name }"
        ...
        <div class="form-group">
          ...
          <input
            ...
            placeholder="Edge 节点 Admin API 密钥（编辑时留空 = 保持原值）"
          />
        </div>
        <div class="form-group">
          <label class="form-label">所属区域</label>
          <select class="form-input" v-model="form.region_code">
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary" :disabled="submitting" @click="handleSubmit">
          {{ submitting ? '保存中...' : editingCluster ? '保存' : '创建' }}
        </button>
      </div>
    </div>
  </div>
...
function initForm(cluster: Cluster | null) {
  if (cluster) {
    ...
    form.region_code = cluster.region_code || ''
...
async function handleSubmit() {
  ...
  try {
    const data = {
      ...
      // 区域：仅选择时提交（空 = 直连；编辑时显式提交 null 才能清除挂接）
      region_code: props.editingCluster ? form.region_code || null : form.region_code || undefined,
    }
...
.form-input:focus {
  border-color: var(--accent);
  box-shadow: 0 0 0 3px oklch(56% 0.16 210 / 12%);
}
.form-input.has-error {
  border-color: #ff4d4f;
}
.form-input.has-error:focus {
  box-shadow: 0 0 0 3px oklch(56% 0.28 27 / 12%);
}
textarea.form-input {
  resize: vertical;
  min-height: 60px;
