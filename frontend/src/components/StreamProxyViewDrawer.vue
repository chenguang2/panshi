        ...
        <div class="modal-body">
          <div v-if="proxy" class="spvd-body">
            ...
            <div class="spvd-field">
              ...
              <span>
                <span v-if="proxy.current_version" class="badge badge-success"
            ...
            <div v-if="proxy.targets && proxy.targets.length > 0" class="spvd-tags">
              <span v-for="(t, i) in proxy.targets" :key="i" class="spvd-tag"
                >{{ t.target }}<span class="spvd-tag-wt">:{{ t.weight }}</span></span
              >
            </div>
...
const isWanEnabled = computed(() => {
  ...
  const cfg = typeof props.proxy.dns_config === 'string' ? JSON.parse(props.proxy.dns_config) : props.proxy.dns_config
